# cpplearning
![](image/readme背景.jpg)

## 作者：张鑫
<img src="image/头像.png" width="30%" />

## 项目内容
balabala...

## 1、firstcpp
这个文件夹里面包含了一些基础的cpp指令包括cout和查看变量的虚拟地址和内存大小的指令

## 2、testvar
这个文件夹里面包含了变量和字面量的定义与运算

## 3、vararea
这个文件夹里面介绍了作用域与生命周期

## 4、testconst
这个文件夹里面介绍了编译时常量和运行时常量

## 5、testauto
这个文件夹介绍了定义类型auto的使用

## 6、iflogic
这个文件里面介绍了if逻辑判断的使用，以及算术运算符，逻辑运算符等使用方法

## 7、test_string
这个文件夹里面介绍了string的用法，以及字符串的特性，截断，空串，字符串到数字stoX转换，数字到字符串to_string转换，字符串的拼接、查找、替换等

## 8、test_enum
这个文件夹里面介绍了枚举类型enum的使用

## 9、test_switch
这个文件夹介绍了switch语句的整型判断和枚举类型判断等方法的使用

## 10、test_main
这个文件夹介绍了main函数的参数传递(int argc, char *argv[], char *env[])

## 11、test_for 
这个文件夹里面介绍了for循环的基本使用、嵌套，还包括break和continue的使用，无限循环、环境变量的打印

## 12、test_while
这个文件夹介绍了while循环的使用，while括号内条件可以使任何表达式，无限循环（占掉一个cpu核）  
设计了一个可以执行简单shell的程序，介绍了system函数的使用  

## 13、test_array
这个文件夹介绍了栈区数组和堆区数组的定义和使用方法，讲解了各自的遍历方法和内存复制方法，以及堆区的释放

## 14、test_vector
1）这个文件夹介绍了vector的初始化，遍历，增加，搜索，删除，排序等内容
2）这个文件夹新增了vector的效率（size、capacity）和插入元素效率（push_back、emplace_back）的内容

